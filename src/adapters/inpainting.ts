...
function imgProcess(img: any): Uint8Array {
...
function markProcess(img: any): Uint8Array {
  const channels = new cv.MatVector()
  cv.split(img, channels) // 分割通道
...
const processImage = withMatCleanup(
  (img: HTMLImageElement, canvasId?: string): Promise<Uint8Array> => {
    return new Promise((resolve, reject) => {
      let src: cv.Mat | null = null
      let src_rgb: cv.Mat | null = null
      try {
        src = cv.imread(img)
        src_rgb = memoryManager.getMat()
        // 将图像从RGBA转换为RGB
        ...
        // 使用内存管理器释放资源
        if (src) memoryManager.releaseMat(src)
        if (src_rgb) memoryManager.releaseMat(src_rgb)
      }
    })
  }
)

const processMark = withMatCleanup(
  (img: HTMLImageElement, canvasId?: string): Promise<Uint8Array> => {
    return new Promise((resolve, reject) => {
      let src: cv.Mat | null = null
      let src_grey: cv.Mat | null = null
      try {
        src = cv.imread(img)
        src_grey = memoryManager.getMat()
        ...
        // 使用内存管理器释放资源
        if (src) memoryManager.releaseMat(src)
        if (src_grey) memoryManager.releaseMat(src_grey)
      }
    })
  }
)
...
function configEnv(capabilities: ImageProcessingCapabilities): void {
